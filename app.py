...
def process_csv(df, df2):
    df = df.copy()
    df = df.drop(
        columns=["attr_smi", "score_min", "score_average", "attr_heavy_atom_count"]
    )
    ...
    pdbid_to_gene = dict(zip(df2["PDBID"].str.lower(), df2["GeneName"]))
    ...
    for r in receptors:
        ...
        if match:
            try:
                df = df.rename(
                    columns={r: upid_to_gene[match.group(1)] + "_" + str(num)}
                )
                gene_names.add(upid_to_gene[match.group(1)])
            except Exception:
                print(
                    f"Warning: No correspondance to any gene name was found for receptor named: {name}"
                )
                gene_names.add(name)
        else:
            try:
...
def merge_csv(csv_list):
    ...
    if len(csv_list) == 1:
        ...
        for i in range(len(csv_list)):
            df_list.append(pd.read_csv(csv_list[i]))
        df_csv_concat = pd.concat(df_list)
        df_csv_concat = df_csv_concat.groupby(
            ["ligand", "collection_key", "scenario"], as_index=False
        ).sum(min_count=1)
...
upfile1 = st.file_uploader(
    "Select your docking result file(s)", accept_multiple_files=True
)
...
if upfile1 and upfile2 is not None:
    ...
    df2["UniProtID"] = [gene_to_upid[i] for i in df2.index.values]
    df2.index.name = "GeneName"

    col_list = list(df2.columns)
    col_list.pop(col_list.index("UniProtID"))

    ligand_options = st.multiselect("Selected ligand ID", col_list, col_list[0])
    ...
    with col1:
        ...
        top = df2[["UniProtID", ligand_option]].sort_values(by=[ligand_option])
        top = top.rename(
            columns={f"{ligand_option}": "Score"}
        ...
        txt = ""
        for g in top["GeneName"].head(cutoff):
    ...
    with col2:
        ...
        with st.form("entry_form"):
            ...
            g = st.text_input(
                "Type the name of genes/UniProt ID you screened (separated by commas):",
                f"{top['GeneName'][0]}",
            ).upper()
            g_list = g.split(",")
            g_list = [x.strip(" ") for x in g_list]

            genre = st.radio("Input type", ("GeneName", "UniProtID"))

            submitted = st.form_submit_button("Query")
            if submitted:
                try:
                    if genre == "GeneName":
                        res = top.loc[top["GeneName"].isin(g_list)]
                    ...
                    elif genre == "UniProtID":
                        res = top.loc[top["UniProtID"].isin(g_list)]
                        st.dataframe(res, use_container_width=True)
                    ...
                    st.write("GeneName/UniProtID not found")
    ...
    if upfile is not None:
        ...
        genre = st.selectbox("True target input type", ("GeneName", "UniProtID"))
        ...
        try:
            ...
            norm = (top["Score"] - top["Score"].max()) / (
                top["Score"].min() - top["Score"].max()
            )
            ...
            st.markdown(
                f"Failed to process the ground truth target file with input type :red[{genre}]"
            )
...
def get_alphafold_data(entry_name):
    ...
    connection = urllib.request.urlopen(
        f"https://alphafold.ebi.ac.uk/files/AF-{entry}-F1-model_v4.cif"
    )
    ...
    confidence = (
        pd.DataFrame(cif.get_category("ma_qa_metric_local"))
